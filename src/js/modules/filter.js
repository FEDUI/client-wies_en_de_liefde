...
filter.posts = function(data) {
    ...
    if ( postsData.description ) {
        ...
        post.description = postsData.description;
        if ( postsData.subattachments ) {
            post.image = postsData.subattachments.data[0].media.image.src;
        }
        if ( postsData.media ) {
...
filter.setPostImages = function(data) {
